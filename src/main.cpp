...
float g_CameraDistance = 16.0f; // Distância da câmera ajustada
...
int main(int argc, char *argv[])
{
    ...
    // de pixels, e com o título exigido pela tarefa.
    GLFWwindow *window;
    window = glfwCreateWindow(800, 600, "INF01047 - 326011 - Leonardo Leal Linhares Dias", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec4 camera_position_c = glm::vec4(x_cam, y_cam, z_cam, 1.0f); // Ponto "c", centro da câmera
        ...
        float farplane = -100.0f; // Posição do "far plane" aumentada para não cortar os coelhos
        ...
        else
        {
            ...
            float t_ortho = 1.5f * g_CameraDistance / 2.5f;
            float b = -t_ortho;
            float right = t_ortho * g_ScreenRatio;
            float l = -right;
            projection = Matrix_Orthographic(l, right, b, t_ortho, nearplane, farplane);
        }
...
#define PLANE 2
        ...
        // Desenhamos o plano do chão (Escalado para cobrir mais espaço)
        model = Matrix_Translate(0.0f, -1.0f, 0.0f) * Matrix_Scale(12.0f, 1.0f, 12.0f);
        glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model));
        glUniform1i(g_object_id_uniform, PLANE);
        DrawVirtualObject("the_plane");

        // Loop principal da cena: 15 Coelhos com Ovos Orbitando
        int num_bunnies = 15;
        float circle_radius = 6.0f;

        for (int i = 0; i < num_bunnies; ++i)
        {
            // Ângulo de distribuição subtraindo a rotação global do tempo -> Movimento Anti-horário
            float angle = (i * 2.0f * 3.141592f / num_bunnies) - (t * 0.5f);
            ...
            glm::mat4 model_bunny_base = Matrix_Translate(bx, by, bz) * Matrix_Rotate_Y(-angle - 3.141592f / 2.0f);
            glm::mat4 model_bunny = model_bunny_base;

            // Backflip 1 a cada 4 coelhos
            if (i % 4 == 0)
            {
                // Rotação no eixo Z (eixo lateral/ombros) para o salto mortal (pitch)
                // Usamos o sinal negativo para rotacionar para trás (backflip).
                // Se ele estiver dando frontflip, mude para positivo (+jump_phase).
                model_bunny = model_bunny * Matrix_Rotate_Z(jump_phase * 2.0f);
            }

            // Envia e desenha Coelho
            glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model_bunny));
            glUniform1i(g_object_id_uniform, BUNNY);
            DrawVirtualObject("the_bunny");

            // Matriz Hierárquica do Ovo 1 (Lado Esquerdo do Coelho)
            ...
            glm::mat4 model_egg1 = model_bunny_base * Matrix_Translate(0.0f, 0.5f, 0.6f) // Move pra esquerda
                                   * Matrix_Rotate_X(orbit_angle1)                       // Órbita da cabeça pra barriga
                                   * Matrix_Translate(0.0f, 0.9f, 0.0f)                  // Raio do círculo que ele faz
                                   * Matrix_Rotate_X(-orbit_angle1)                      // Anula a rotação
                                   * Matrix_Scale(0.2f, 0.3f, 0.2f);                     // Achata para parecer ovo

            glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model_egg1));
            glUniform1i(g_object_id_uniform, SPHERE);
            DrawVirtualObject("the_sphere");

            // Matriz Hierárquica do Ovo 2 (Lado Direito do Coelho)
            // Lado do coelho fica no eixo X. Usaremos +0.6f.
            glm::mat4 model_egg2 = model_bunny_base * Matrix_Translate(0.0f, 0.5f, -0.6f) // Move pra direita
                                   * Matrix_Rotate_X(orbit_angle2)                        // Órbita sincronizada
                                   * Matrix_Translate(0.0f, 0.9f, 0.0f)                   // Raio do círculo que ele faz
                                   * Matrix_Rotate_X(-orbit_angle2)                       // Anula a rotação
                                   * Matrix_Scale(0.2f, 0.3f, 0.2f);                      // Achata para parecer ovo

            glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model_egg2));
            glUniform1i(g_object_id_uniform, SPHERE);
            DrawVirtualObject("the_sphere");
        }
